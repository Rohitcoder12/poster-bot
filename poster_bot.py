...
# --- Default link domains (used to create the domains file if it doesn't exist) ---
...
# --- Persistence for allowed domains ---
...
def upload_to_imagebb(image_path):
    if not IMAGEBB_API_KEY:
        logger.warning("IMAGEBB_API_KEY not set. Skipping upload.")
        return None
    try:
...
def process_and_publish_post(context: CallbackContext, title: str, caption_text: str, photo_path: str, links_list: list, user_name: str, source: str, chat_id=None):
    try:
        ...
        if not service:
            ...
            if source == 'manual' and chat_id:
                context.bot.send_message(chat_id=chat_id, text="Error: Could not connect to Google. Admin has been notified.")
            return
        image_url = None
        if photo_path and os.path.exists(photo_path):
            ...
            if not image_url:
                ...
                if source == 'manual' and chat_id:
                    context.bot.send_message(chat_id=chat_id, text="Error: Failed to upload the image. Please try again.")
                return
        ...
        if source == 'manual' and chat_id:
            context.bot.send_message(chat_id=chat_id, text=f"Success! Post '{title}' published.")
    except Exception:
        error_details = traceback.format_exc()
        send_log(f"❌ {source.upper()} ERROR! Failed to post '{title}'.\n\n{error_details}", is_error=True)
        if source == 'manual' and chat_id:
            context.bot.send_message(chat_id=chat_id, text=f"An unexpected error occurred. Admin has been notified.")
    finally:
        if photo_path and os.path.exists(photo_path):
...
def channel_post_handler(update: Update, context: CallbackContext):
    ...
    if not post or post.chat_id not in SOURCE_CHANNEL_IDS or not (post.photo or post.video):
        return

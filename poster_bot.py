...
ADMIN_ID = os.environ.get("ADMIN_ID") # NEW: Your Telegram User ID
...
# --- NEW: Default link domains (from your image and examples) ---
# This list is used to create the domains file if it doesn't exist.
...
# --- NEW: Persistence for allowed domains ---
...
def load_domains():
    """Loads domains from the file, or creates the file from defaults."""
    if os.path.exists(DOMAINS_FILE):
        with open(DOMAINS_FILE, "r") as f:
            # Use a set for fast lookups and no duplicates
...
def save_domains(domains_set):
    """Saves the current set of domains to the file."""
...
# ... (all helper functions like send_log, get_blogger_service, etc., remain here) ...
# For brevity, I'm omitting the unchanged functions. Please copy them from the previous code block.
...
def process_and_publish_post(context: CallbackContext, title: str, caption_text: str, photo_path: str, links_list: list, user_name: str, source: str, chat_id=None):
    try:
        ...
        if not service:
            ...
            if source == 'manual' and chat_id: context.bot.send_message(chat_id=chat_id, text="Error: Could not connect to Google. Admin has been notified.")
            return
        image_url = None
        if photo_path and os.path.exists(photo_path):
            image_url = upload_to_imagebb(photo_path)
            if not image_url:
                log_msg = f"❌ {source.upper()} ERROR! Image upload to ImageBB failed for post '{title}'."
                send_log(log_msg, is_error=True)
                if source == 'manual' and chat_id: context.bot.send_message(chat_id=chat_id, text="Error: Failed to upload the image. Please try again.")
                return
        ...
        if source == 'manual' and chat_id: context.bot.send_message(chat_id=chat_id, text=f"An unexpected error occurred. Admin has been notified.")
    finally:
...
def channel_post_handler(update: Update, context: CallbackContext):
    ...
    if not post or post.chat_id not in SOURCE_CHANNEL_IDS or not (post.photo or post.video): return
    chat_title = post.chat.title or f"ID:{post.chat_id}"
    send_log(f"AUTOMATION: Detected new media in channel '{chat_title}'.")
    ...
    if not valid_urls:
        send_log(f"AUTOMATION: Post in '{chat_title}' ignored. No valid links found matching domains: {', '.join(VALID_LINK_DOMAINS)}")
        return
    ...
    for pattern in junk_patterns: clean_caption = re.sub(pattern, '', clean_caption, flags=re.IGNORECASE)
    main_caption = "\n".join([line.strip() for line in clean_caption.split('\n') if line.strip()])
    if not main_caption:
        title = f"New Post from {chat_title}"
        send_log(f"AUTOMATION: Could not extract a clean caption. Using fallback title: '{title}'")
    else:
...
def cancel(update: Update, context: CallbackContext) -> int:
    ...
    if 'photo_path' in context.user_data and context.user_data['photo_path'] and os.path.exists(context.user_data['photo_path']):
...
def add_site(update: Update, context: CallbackContext):
    ...
    if domain_to_add in VALID_LINK_DOMAINS:
        ...
        send_log(f"ADMIN: {update.effective_user.first_name} added new domain: {domain_to_add}")
...
def remove_site(update: Update, context: CallbackContext):
    ...
    if domain_to_remove in VALID_LINK_DOMAINS:
        ...
        send_log(f"ADMIN: {update.effective_user.first_name} removed domain: {domain_to_remove}")
    else:
...
def list_sites(update: Update, context: CallbackContext):
    ...
    if not VALID_LINK_DOMAINS:
        update.message.reply_text("The list of allowed domains is currently empty.")
    else:
        message = "📝 **Currently Allowed Domains:**\n\n"
        # Using a code block for better formatting
...
def help_command(update: Update, context: CallbackContext):
    ...
    help_text = """
Hello! I am your Blogger Poster Bot. Here's what I can do:

**User Commands:**
/start - Begin a conversation to manually create a new blog post.
/cancel - Stop the current manual posting process.
...
"""
    ...
    admin_help_text = """
**Admin Commands:**
/addsite `<domain.com>` - Add a new website domain to the automation whitelist.
/removesite `<domain.com>` - Remove a domain from the whitelist.
/listsites - Show all currently whitelisted domains.
"""
...
# Conversation handler for manual posting
...
if __name__ == "__main__":
    if WEBHOOK_URL:
        ...
        send_log("🚀 Bot has been deployed/restarted with new site management features.")
    else:
        logger.warning("WEBHOOK_URL not set. Bot started but will not receive updates.")
